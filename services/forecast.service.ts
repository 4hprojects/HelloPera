...
/**
 * §52 — the dashboard's single lightweight number.
 *
 * Separate from `getForecast` so the dashboard does not pay for a full
 * timeline it will not draw, and does not trigger generation on every
 * dashboard load.
 */
export async function getProjectedBalance(
  ...
  opening: ReturnType<typeof zero>;
  closing: ReturnType<typeof zero>;
} | null> {
  const end = addDays(today, 30);

  const [accounts, bills, expectedIncome, events] = await Promise.all([
    listAccounts(),
    listObligations('bill', today, { onlyOpen: true, from: today, to: end }),
    listObligations('expected_income', today, { onlyOpen: true, from: today, to: end }),
    getUpcomingExpectedEvents(today, end),
  ...
  const forecastEvents: ForecastEvent[] = [
    ...bills
      .filter((b) => b.currency === currency && b.date)
      .map((b) => ({
        id: `bill:${b.id}`,
        date: b.date!,
        kind: 'bill' as const,
    ...
    ...expectedIncome
      .filter((e) => e.currency === currency && e.date)
      .map((e) => ({
        id: `expected_income:${e.id}`,
        date: e.date!,
    ...
    ...events
      .filter((e) => e.amount.currency === currency && includesEvent(e))
      .map((e) => ({
        id: `event:${e.id}`,
        ...
        label: e.name,
        amount: e.amount,
        direction: (e.eventType === 'income' ? 'in' : 'out') as 'in' | 'out',
        confidence: confidenceFor('expected_event'),
        href: null,
      })),
  ];
