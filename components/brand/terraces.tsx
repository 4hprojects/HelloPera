/**
 * Layered mountain bands — rice terraces at first light, the showcase's hero.
 *
 * Inline SVG rather than a raster: it is four gradient ridges and a sun, it
 * scales to any width without a second asset, it costs nothing to load, and it
 * re-tints for dark mode from the same markup.
 *
 * `aria-hidden` and always behind text — decorative, never carrying meaning.
 *
 * Extracted from `components/dashboard/hero-banner.tsx` so the dashboard and
 * the landing page draw the same horizon. A second hand-tuned copy would have
 * drifted, and the two are the first and last thing a user sees.
 */
